(function () {
  ...
  function initScrollLockedVideo(section, video, caption) {
    ...
    // Секции высокие (много vh), поэтому нельзя ориентироваться на долю
    // площади ВСЕЙ секции (intersectionRatio) — она физически никогда не
    // достигнет высокого порога. Вместо этого «сжимаем» root до узкой
    // полосы по центру экрана: как только секция долетает до неё —
    // считаем, что видео «на экране», и запускаем его.
